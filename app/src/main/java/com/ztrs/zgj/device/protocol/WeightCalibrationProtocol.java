...
public class WeightCalibrationProtocol extends BaseProtocol{
    private static final String TAG= WeightCalibrationProtocol.class.getSimpleName();

    public static final byte CMD_WEIGHT_CALIBRATION = (byte) 0x38;//载重标定

    private DeviceOperateInterface deviceOperateInterface;

    public WeightCalibrationProtocol(DeviceOperateInterface deviceOperateInterface){
        super(TAG);
    ...
    private boolean parseData(byte[] data) {
        ...
        int i= 0;
        WeightCalibrationBean calibrationBean = deviceOperateInterface.getZtrsDevice().getWeightCalibrationBean();
    ...
    public void weightCalibration(WeightCalibrationBean calibrationBean){
        ...
        byte[] cmd = CommunicationProtocol.packetCmd(CMD_WEIGHT_CALIBRATION,data);
        deviceOperateInterface.sendDataToDevice(cmd);
        WeightCalibrationMessage msg = new WeightCalibrationMessage(CommunicationProtocol.seq++,cmd);
        msg.setCmdType(BaseMessage.TYPE_CMD);
        cmdSend(msg);
    }
    public void queryWeightCalibration(){
        byte[] cmd = CommunicationProtocol.packetCmd(CMD_WEIGHT_CALIBRATION,new byte[]{0});
        deviceOperateInterface.sendDataToDevice(cmd);
        WeightCalibrationMessage msg = new WeightCalibrationMessage(CommunicationProtocol.seq++,cmd);
        cmdSend(msg);
    }
